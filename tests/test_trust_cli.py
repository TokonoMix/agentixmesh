...
def test_uid_project_override_is_restrict_only(tmp_path, monkeypatch):
    p = _policy(tmp_path, monkeypatch)
    trust_cli.main(["grant", "1100", "notify-only"])
    trust_cli.main(["grant", "1100", "human-gate", "--project", "pm-mesh"])  # stricter → honored
    data = json.loads(p.read_text())
    assert trust.resolve(data, 1100, "pm-mesh", 1200) == "human-gate"
    # a looser project override would be ignored by the engine
    trust_cli.main(["grant", "1100", "human-gate"])
    trust_cli.main(["grant", "1100", "notify-only", "--project", "other"])  # looser → ignored
    data = json.loads(p.read_text())
    assert trust.resolve(data, 1100, "other", 1200) == "human-gate"


def test_revoke_returns_to_default(tmp_path, monkeypatch):
    p = _policy(tmp_path, monkeypatch)
    trust_cli.main(["grant", "1100", "notify-only"])
    trust_cli.main(["revoke", "1100"])
    data = json.loads(p.read_text())
    assert "1100" not in data["by_uid"]
    assert trust.resolve(data, 1100, "pm-mesh", 1200) == "human-gate"  # back to cross-user default
