...
EOF
sudo tee /usr/local/bin/mesh-onboard-agent >/dev/null <<'EOF'
#!/usr/bin/env sh
# mesh-onboard-agent <harness> probe|wire|register-delivery — wire a NON-Claude harness
# (codex | gemini | openclaw) onto the mesh and emit a verifiable provenance manifest.
: "${MESH_ROOT:=/srv/mesh}"; export MESH_ROOT
exec python3 -m pm_mesh.harness_onboard "$@"
EOF
